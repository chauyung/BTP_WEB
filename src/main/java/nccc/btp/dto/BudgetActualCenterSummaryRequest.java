...
public class BudgetActualCenterSummaryRequest {
    ...
    private List<Integer> expandLevels = new ArrayList<>(Arrays.asList(1, 4, 6, 8));
    private SourceMode sourceMode = SourceMode.BOTH;

    public void validate() {
        if (!StringUtils.hasText(startYm) || !StringUtils.hasText(endYm))
            throw new IllegalArgumentException("年月起訖必填");
        if (startYm.compareTo(endYm) > 0)
            throw new IllegalArgumentException("起值不可大於迄值");
        parseYm(startYm);
        parseYm(endYm);

        if (expandLevels == null || expandLevels.isEmpty())
            expandLevels = new ArrayList<>(Arrays.asList(1, 4, 6, 8));
        for (Integer lv : expandLevels)
            if (!(lv == 1 || lv == 4 || lv == 6 || lv == 8))
                throw new IllegalArgumentException("展開層級只支援 1/4/6/8 碼");

        if (sourceMode == null) sourceMode = SourceMode.BOTH;

        if (StringUtils.hasText(accountFrom) && StringUtils.hasText(accountTo)
                && accountFrom.compareTo(accountTo) > 0)
            throw new IllegalArgumentException("科目代號起不可大於迄");
    }

    private static void parseYm(String yyyymm) {
        if (yyyymm == null || yyyymm.length() != 6)
            throw new IllegalArgumentException("年月需為 6 碼");
